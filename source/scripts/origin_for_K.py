...
# 修复Windows路径转义问题 + 统一路径格式
csv_path = r"D:\code\Algorithm-Design-and-Analysis\source\data\k_benchmark.csv"
...
plt.title("n=10000 时不同k值对全排序算法效率的影响", fontsize=14, pad=20)
plt.xlabel("Top-K 值 k", fontsize=12)
plt.ylabel("平均运行耗时 (ms)", fontsize=12)
plt.legend(fontsize=11)
plt.grid(alpha=0.3)
plt.tight_layout()

# ================== 保存图片 ==================
plt.savefig(os.path.join(save_dir, "FullSort_All_Algorithm_Compare.png"))
plt.close()

print("✅ 全排序多算法对比图绘制完成，已保存到：", os.path.abspath(save_dir))